...
def view_action(request, action_id):
	...
	if action.attendees.filter(pk=current_user):
		participating = True
	else:
		participating = False
	if request.method == "POST":
		# you can only join an event in you're not the organizer
		if current_user != action.organizer.id and not participating:
			action.attendees.add(current_user)
			participating = True
	context = {'action': action, 'organizer': organizer, 'logged_as' : request.user.id, 'participating' : participating}
